      ...
      <el-form ref="form" :model="data.form" label-width="70px" style="padding: 20px">
        ...
        <el-form-item prop="sellPet" label="销售宠物">
          <el-select multiple style="width: 100%" v-model="data.sellPet">
            <el-option v-for="item in data.virtual_gameTypeList" :key="item.id" :value="item.name" :label="item.name"></el-option>
          </el-select>
        </el-form-item>
...
const data = reactive({
  ...
  ids: [],
  sellPet: [],
  virtual_gameTypeList: []
})

request.get('/virtual_gameType/selectAll').then(res => {
  data.virtual_gameTypeList = res.data
})

const load = () => {
  request.get('/virtual_gameShop/selectPage', {
    params: {
...
const add = () => {
  request.post('/virtual_gameShop/add', data.form).then(res => {
    if (res.code === '200') {
...
const audit = (row, status) => {
  ElMessageBox.confirm('您确认' + status + '吗？', '审核确认', { type: 'warning' }).then(res => {
    ...
    form.status = status
    request.put('/virtual_gameShop/identify', form).then(res => {
      if (res.code === '200') {
        ElMessage.success('操作成功')
        data.formVisible = false
        load()
...
const update = () => {
  request.put('/virtual_gameShop/update', data.form).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
      data.formVisible = false
      load()
...
const del = (id) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗？', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/virtual_gameShop/delete/' + id).then(res => {
      if (res.code === '200') {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗？', '删除确认', { type: 'warning' }).then(res => {
    request.delete("/virtual_gameShop/delete/batch", {data: data.ids}).then(res => {
      if (res.code === '200') {
        ElMessage.success('操作成功')
        load()
      } else {
