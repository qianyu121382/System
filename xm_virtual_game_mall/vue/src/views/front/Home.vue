    ...
    <div style="margin-bottom: 20px">
      <el-carousel height="400px">
        <el-carousel-item v-for="item in data.slideshowList" :key="item">
          <img @click="goTo('/front/virtual_gameDetail?id=' + item.virtual_gameId)" :src="item.img" alt="" style="width: 100%; height: 400px; cursor: pointer">
        </el-carousel-item>
      </el-carousel>
    </div>
        ...
        <div>
          <el-row :gutter="20">
           <el-col :span="12" v-for="item in data.recommendPetList" :key="item.id">
             <div @click="goTo('/front/virtual_gameDetail?id=' + item.id)" class="card" style="padding: 0; margin-bottom: 20px;
...
request.get('/virtual_game/selectAll', {
  params: {
    recommend: '是',
    status: '上架'
  }
...
request.get('/virtualGameShop/selectRank').then(res => {
  data.virtual_gameShopList = res.data
})
