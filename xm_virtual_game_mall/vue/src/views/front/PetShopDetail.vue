      ...
      <div style="padding-top: 20px" v-if="data.total > 0">
        <el-row :gutter="20">
         <el-col :span="6" v-for="item in data.virtual_gameList" :key="item.id">
           <div style="cursor: pointer" @click="router.push('/front/virtual_gameDetail?id=' + item.id)">
...
request.get('/virtual_gameShop/selectById/' + data.id).then(res => {
  data.shop = res.data
})
...
const loadPetList = () => {
  ...
  request.get('/virtual_game/selectPage', {
    ...
    data.virtual_gameList = res.data?.list
