  ...
  <div style="width: 60%; margin: 10px auto; min-height: 50vh">
    ...
    <div style="margin: 20px 0" v-if="data.total > 0">
      <div class="card" style="padding: 20px; margin-bottom: 10px; display: flex; align-items: center" v-for="item in data.tableData" :key="item.id">
        <div @click="router.push('/front/virtual_gameShopDetail?id=' + item.id)" style="flex: 1; display: flex; grid-gap: 20px; cursor: pointer;">
        ...
        <div style="flex: 1">
          <el-row :gutter="20">
           <el-col :span="6" v-for="virtual_game in item.virtual_gameList" :key="virtual_game.id">
             <img @click="router.push('/front/virtual_gameDetail?id=' + virtual_game.id)" :src="virtual_game.img" alt="" style="width: 100px; height: 100px; border-radius: 50%; cursor: pointer">
           </el-col>
          </el-row>
        </div>
      </div>
