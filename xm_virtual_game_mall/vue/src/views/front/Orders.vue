...
const buyAgain = (row) => {
  request.post('/virtual_gameOrders/add', row).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
      load()
    } else {
...
const confirm = (row) => {
  data.form = JSON.parse(JSON.stringify(row))
  data.form.status = '已完成'
  ElMessageBox.confirm('您确认签收【 ' + row.virtual_gameName + '】吗？', '签收确认', { type: 'warning' }).then(res => {
    request.put('/virtual_gameOrders/update', data.form).then(res => {
      if (res.code === '200') {
        ElMessage.success('签收成功')
        load()
      } else {
...
const cancel = (row) => {
  data.form = JSON.parse(JSON.stringify(row))
  data.form.status = '已取消'
  ElMessageBox.confirm('您确认取消【 ' + row.virtual_gameName + '】订单吗？', '取消确认', { type: 'warning' }).then(res => {
    request.put('/virtual_gameOrders/update', data.form).then(res => {
      if (res.code === '200') {
        ElMessage.success('取消成功')
        load()
...
const load = () => {
  request.get('/virtual_gameOrders/selectPage', {
    params: {
      pageNum: data.pageNum,
      pageSize: data.pageSize,
      virtual_gameName: data.virtual_gameName,
...
const loadGoods = () => {
  request.get('/goodsOrders/selectPage', {
    params: {
      ...
      virtual_gameName: data.goodsName,
...
const add = () => {
  request.post('/virtual_gameOrders/add', data.form).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
...
const del = (id) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗？', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/virtual_gameOrders/delete/' + id).then(res => {
      if (res.code === '200') {
