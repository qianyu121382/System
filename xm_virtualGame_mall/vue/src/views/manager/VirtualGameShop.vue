...
request.get('/virtualGameType/selectAll').then(res => {
  data.virtualGameTypeList = res.data
})
...
const add = () => {
  request.post('/virtualGameShop/add', data.form).then(res => {
    if (res.code === '200') {
...
const audit = (row, status) => {
  ElMessageBox.confirm('您确认' + status + '吗？', '审核确认', { type: 'warning' }).then(res => {
    ...
    form.status = status
    request.put('/virtualGameShop/identify', form).then(res => {
      if (res.code === '200') {
        ElMessage.success('操作成功')
        data.formVisible = false
        load()
...
const update = () => {
  request.put('/virtualGameShop/update', data.form).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
      data.formVisible = false
      load()
...
const del = (id) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗？', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/virtualGameShop/delete/' + id).then(res => {
      if (res.code === '200') {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗？', '删除确认', { type: 'warning' }).then(res => {
    request.delete("/virtualGameShop/delete/batch", {data: data.ids}).then(res => {
      if (res.code === '200') {
        ElMessage.success('操作成功')
        load()
      } else {
