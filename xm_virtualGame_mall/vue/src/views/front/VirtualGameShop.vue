  ...
  <div style="width: 60%; margin: 10px auto; min-height: 50vh">
    ...
    <div style="margin: 20px 0" v-if="data.total > 0">
      <div class="card" style="padding: 20px; margin-bottom: 10px; display: flex; align-items: center" v-for="item in data.tableData" :key="item.id">
        <div @click="router.push('/front/virtualGameShopDetail?id=' + item.id)" style="flex: 1; display: flex; grid-gap: 20px; cursor: pointer;">
        ...
        <div style="flex: 1">
          <el-row :gutter="20">
           <el-col :span="6" v-for="virtualGame in item.virtualGameList" :key="virtualGame.id">
             <img @click="router.push('/front/virtualGameDetail?id=' + virtualGame.id)" :src="virtualGame.img" alt="" style="width: 100px; height: 100px; border-radius: 50%; cursor: pointer">
           </el-col>
          </el-row>
        </div>
      </div>
...
const load = () => {
  request.get('/virtualGameShop/selectPage', {
    params: {
