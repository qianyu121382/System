      ...
      <div style="margin: 20px 0" v-if="data.total > 0">
        <el-row :gutter="20">
          <el-col :span="6" v-for="item in data.tableData" :key="item.id">
            <div @click="router.push('/front/virtualGameDetail?id=' + item.fid)" style="padding: 0; margin-bottom: 20px;
                cursor: pointer; border-radius: 5px; overflow: hidden" >
              <img :src="item.virtualGameImg" alt="" style="width: 100%; height: 220px; display: block">
              <div style="margin-top: 10px">
                <div style="font-size: 20px; font-weight: bold; margin-bottom: 10px" class="line1">{{ item.virtualGameName }}</div>
                <div style="font-size: 20px; color: red">￥{{ item.virtualGamePrice }}</div>
              </div>
            </div>
          </el-col>
        </el-row>
